...
def grad(K, As, Bs):
    ...
    # jax.debug.print("grads: {}", jnp.mean(grads, axis=0))
...
def grad_risk(K, As, Bs, t=1.0):
    def single_grad(A, B):
        ...
        return 2*E@S
    grads = vmap(single_grad, (0, 0))(As, Bs)
    weights = jax.nn.softmax(t*cost(K, As, Bs)[0])
    avg_grads_risk = jnp.tensordot(weights, grads, axes=1)

    # weights = jnp.exp(t*cost(K, As, Bs)[0])
    # weighted_grads = jnp.tensordot(weights, grads, axes=1)
    # avg_grads_risk = weighted_grads / jnp.sum(weights)
    
    # Correct debug prints
    # jax.debug.print("costs: {}", cost(K, As, Bs)[0])
    # jax.debug.print("weights: {}", weights)
    # jax.debug.print("grads: {}", jnp.mean(grads, axis=0))    
    # jax.debug.print("weighted_grads: {}", weighted_grads)
    # jax.debug.print("avg_grads_risk: {}", avg_grads_risk)
    
    return avg_grads_risk

def grad_descent_risk(K, As, Bs, alpha, n_iterations):
    def body_fn(i, K):
        grad_k = grad_risk(K, As, Bs)  # Use grad directly instead of avg_grad
        K = K - alpha*grad_k
        return K
...
def update_gamma(gamma, As, Bs, K):
  C_old = cost(K, jnp.sqrt(gamma)*As, jnp.sqrt(gamma)*Bs)[1]
  ...
  while gamma_ub - gamma_lb > 1e-4:
    ...
    C_new = cost(K, jnp.sqrt(gamma_mid)*As, jnp.sqrt(gamma_mid)*Bs)[1]
...
for i, n_samples in enumerate([10, 20, 500]):
  all_Ks = []
  all_SA_cost = []
  all_DR_cost = []
  ...
  for trial in range(n_trials):
    ...
    while gamma < 0.9999:
      K = grad_descent_risk(K, jnp.sqrt(gamma)*As, jnp.sqrt(gamma)*Bs, alpha, n_iterations)
      Ks.append(K); SA_cost.append(cost(K, As, Bs)[1]); DR_cost.append(cost(K, eval_As, eval_Bs)[1])
      print('K: ', K)
    #   print('A + B@K', [A+B@K for (A,B) in zip(As,Bs)])
      print('iteration: ', len(Ks))
      gamma = update_gamma(gamma, As, Bs, K)
      print('gamma: ', gamma)
      print('SA_cost: ', cost(K, As, Bs)[1])
      print('DR_cost: ', cost(K, eval_As, eval_Bs)[1])
    ...
    for i in range(20):
      K = grad_descent_risk(K, As, Bs, alpha, n_iterations)
      Ks.append(K); SA_cost.append(cost(K, As, Bs)[1]); DR_cost.append(cost(K, eval_As, eval_Bs)[1] )
      print('iteration: ', len(Ks))
      print('K: ', K)
...
# save the results
with open(f'stabilization_results_risk_{n_trials}.pkl', 'wb') as f:
  pickle.dump({
